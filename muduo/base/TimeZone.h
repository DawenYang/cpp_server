...
namespace muduo {
    ...
    public:
        TimeZone() = default;
        TimeZone(int eastOfUat, const char *tzname);
        ...
        struct DateTime toLocalTime(int64_t secondSinceEpoch, int *utcOffset = nullptr) const;
        int64_t fromLocalTime(const struct DateTime &, bool postTransition = false) const;

        static struct DateTime toUtcTime(int64_t secondSinceEpoch);
